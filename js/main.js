...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const addHighlightTool = function () {
    ...
    // 在调用处添加文章内容容器的判断
    const articleContentSelector = '.post-content'; // 根据实际文章容器调整选择器
    
    if (isPrismjs) {
      $figureHighlight.forEach(item => {
        const isInArticle = item.closest(articleContentSelector) !== null;
        if (!isInArticle) return; // 不在文章中则跳过
        if (highlightLang) {
  ...
  const generateHeadingIds = () => {
    ...
    document.querySelectorAll('#article-container h1, h2, h3, h4, h5, h6').forEach((heading, index) => {
      // 跳过已有ID的标题
      if (heading.id && heading.id !== 'CrawlerTitle') return;
      ...
      // 应用ID（兼容中文需URI编码）
      heading.id = encodeURIComponent(uniqueId);
    });
  };
  ...
  const updateTocLinks = () => {
    ...
    tocLinks.forEach(link => {
      const text = link.querySelector('.toc-text').textContent;
      const heading = Array.from(document.querySelectorAll('#article-container h1, h2, h3, h4, h5, h6'))
        .find(h => decodeURIComponent(h.id) === generateIdFromText(text));
      
      if (heading) {
        link.href = `#${encodeURIComponent(heading.id)}`;
      } else {
        link.href = `#${encodeURIComponent(generateIdFromText(text))}`;
        console.warn('自动修复缺失的目录链接:', link);
      }
    });
  };
  ...
  // 新增：目录链接验证（可选）
  const validateTocLinks = () => {
    document.querySelectorAll('.toc-link').forEach(link => {
      const targetId = decodeURIComponent(link.href.split('#')[1] || '');
      if (!document.getElementById(targetId)) {
        console.error('无效目录链接:', link);
        link.style.borderLeft = '2px solid red';
      }
    });
  };
    // -------------------------------------------
    ...
    const findHeadPosition = function (top) {
      ...
      if (isToc) {
        ...
        const currentActive = $tocLink[currentIndex-1];
